...
let calcDeckStats = async () => {
	...
	const decks = await client.query(
		"SELECT deck_url FROM mtg.tournament_decks WHERE cards <> '{}' AND cards IS NOT NULL AND unknown_cards_main = FALSE AND NOT EXISTS (SELECT 1 FROM mtg.deck_stats WHERE deck_stats.deck_url = tournament_decks.deck_url);"
	);
	...
	for (const deck of rows) {
		...
		const cardsResponse = await client.query('SELECT UNNEST(cards) FROM mtg.tournament_decks WHERE deck_url = $1;', [deckUrl]);
		var stats = new Map([
			['main', 0],
			['sideboard', 0],
			['layout', new Map()],
